...
for filei=1: length(files2import)
    ...
    alldataC{filei} = randomdata ; % curly brackets are for cells 
    
    % for matrix storage, initialize on first iteration ; we do not want to
    % keep overwriting with each loop. This is because everything would be
    % zero except for the last iteration. 
    if filei == 1 %remember filei indicates the first page of the matrix 
